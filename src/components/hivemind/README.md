# Hivemind - hivemind.davinciai.eu

This folder contains all components and pages for the **Hivemind** domain (`hivemind.davinciai.eu`).
...
├── HivemindRedirect.jsx    # Route component that redirects to hivemind.davinciai.eu
...
The route `/hivemind` is configured in `App.js` to redirect to `https://hivemind.davinciai.eu`.
...
To serve this content on `hivemind.davinciai.eu`:

### Option A: Direct Navigation (Current Setup)
- User clicks button → redirects to `https://hivemind.davinciai.eu`
...
1. Add `hivemind.davinciai.eu` in Vercel Project Settings > Domains
...
export function middleware(request) {
  ...
  if (hostname === 'hivemind.davinciai.eu') {
