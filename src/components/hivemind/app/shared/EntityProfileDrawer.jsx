  ...
  const content = state.loading ? <p className="py-12 text-center text-sm text-[#78716c]">Loading evidence…</p>
    : state.error ? <p className="py-12 text-center text-sm text-[#78716c]">{state.error}</p>
      : <>
        ...
        <div className="space-y-6 p-5">{state.dossier.facts.length === 0 ? (
          <div className="rounded-xl border border-dashed border-[#d9d4cc] bg-[#faf9f5] p-5 text-sm text-[#78716c]">
            <p className="font-medium text-[#57534e]">No verified profile facts yet.</p>
            <p className="mt-1 leading-relaxed">HIVE has identified this entity, but it has no evidence-backed facts ready to show. New candidate facts appear here after canonical projection and, in review mode, approval.</p>
          </div>
        ) : ['static', 'dynamic', 'relationship', 'historical'].map((factClass) => {
