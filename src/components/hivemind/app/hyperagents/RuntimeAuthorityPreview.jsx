...
import {
  Bookmark, Heart, Linkedin, Mail, MessageCircle, MoreHorizontal,
...
function Meta({ action, label }) {
  return <div className="mt-3 flex items-center justify-between gap-3 font-mono text-[8px] uppercase tracking-[0.1em] text-[#85817b]">
    <span>{label}</span>
    <span>{action.scheduled_at ? new Date(action.scheduled_at).toLocaleString() : 'After approval'}</span>
  </div>;
}
...
export function GmailMessagePreview({ message, action }) {
  const payload = action?.payload || {};
  ...
  return <article className="mx-auto w-full max-w-[620px] overflow-hidden rounded-[8px] border border-[#d7dbe3] bg-white shadow-sm" aria-label="Gmail message preview">
    ...
    <div className="flex items-center border-t border-[#eceff3] px-4 py-3"><span className="inline-flex h-8 items-center gap-2 rounded-full bg-[#0b57d0] px-5 text-[12px] font-semibold text-white">Send <span className="text-[9px]">▼</span></span><span className="ml-auto text-[10px] text-[#6b6f75]">Preview only</span></div>
  </article>;
}
...
export function CampaignLaunchPreview({ campaign }) {
  ...
  return <section aria-label="Campaign launch batch">
    ...
    <div className="space-y-5">{actions.map((action) => {
      const channel = String(action.channel || '').toLowerCase();
      if (channel === 'x' || channel === 'twitter' || channel === 'x_organic') return <XPostPreview key={action.id} action={action} accountName={accountName} />;
      if (channel === 'instagram') return <InstagramPostPreview key={action.id} action={action} accountName={accountName} />;
      if (channel === 'linkedin') return <LinkedInPostPreview key={action.id} action={action} accountName={accountName} />;
      if (channel === 'gmail') return <GmailMessagePreview key={action.id} action={action} />;
      return <GenericPostPreview key={action.id} action={action} accountName={accountName} />;
    })}</div>
  </section>;
}
...
export function AuthorityReviewContent({ approval }) {
  return <div className="space-y-5">
    ...
    {(approval.calls || []).map((call, index) => <article key={`${call.id || call.phone || 'call'}-${index}`} className="mx-auto w-full max-w-[620px] overflow-hidden rounded-[8px] border border-[#d8d3cc] bg-white"><div className="flex items-center gap-3 border-b border-[#ebe8e3] px-4 py-3"><span className="grid h-9 w-9 place-items-center rounded-full bg-[#171717] text-white"><Phone size={16} /></span><div className="min-w-0"><div className="truncate text-[13px] font-semibold">{call.prospect || call.phone}</div><div className="font-mono text-[9px] text-[#777168]">{call.phone}</div></div></div><div className="space-y-3 px-4 py-4 text-[12px] leading-5 text-[#525252]"><p><strong className="text-[#171717]">Goal</strong><br />{call.goal}</p>{call.opener ? <p><strong className="text-[#171717]">Opening</strong><br />{call.opener}</p> : null}{call.strategy ? <p><strong className="text-[#171717]">Strategy</strong><br />{call.strategy}</p> : null}</div></article>)}
