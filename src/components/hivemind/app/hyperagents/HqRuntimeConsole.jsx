...
import { AuthorityReviewContent, CallPreview, CampaignLaunchPreview, GmailMessagePreview, PlatformActionPreview, RuntimeCampaignCanvas } from './RuntimeAuthorityPreview';
...
export function AgentRuntimeTasksPanel({ queue, growthBrief, firstLife, experience, onDecision, onAdminDecision, onClose }) {
  ...
  return <section id="agent-runtime-tasks" className="relative w-full overflow-hidden rounded-[14px] border border-[#e3e0db] bg-white shadow-[0_14px_36px_-30px_rgba(0,0,0,0.55)]" aria-label="Artifacts and Runtime tasks">
    ...
    <div className="max-h-[min(72vh,680px)] overflow-y-auto">
      ...
      {/* The admin check-in is NOT a task-list card. It presents itself as the call
          popup (auto-opened while phase === AWAITING_ADMIN_CHECKIN), where the only two
          choices live: Start the call, or Skip. Nothing to hunt for in this panel. */}
      {queue.map((item) => {
...
export function NarrativeEvent({ item, active }) {
  ...
  // Campaign progress events are represented by the live post canvas. Keeping the
  // event persisted drives SSE refresh without duplicating it as narration.
  if (item.eventType === 'campaign_artifact_progress') return null;
...
export default function HqRuntimeConsole({ objective, baselineReady }) {
  ...
  // Claude-style stick-to-bottom: while the newest content is in view, new events
  // auto-scroll the page to keep following; if the reader scrolls up to read history,
  // we stop following and never yank them back down.
  ...
  // screen the reader is at the bottom → stay stuck; once they scroll up to read back,
  // it leaves the viewport → stop following. rootMargin gives a small grace band so a
  // near-bottom reader still counts as stuck (Claude-style).
  useEffect(() => {
    const sentinel = transcriptBottomRef.current;
    if (!sentinel || typeof IntersectionObserver === 'undefined') return undefined;
    const observer = new IntersectionObserver(
      (entries) => { stuckToBottomRef.current = entries[0]?.isIntersecting ?? true; },
      { root: null, rootMargin: '0px 0px 120px 0px', threshold: 0 },
    );
    observer.observe(sentinel);
    return () => observer.disconnect();
  }, []);
  // On new streamed content, if the reader is stuck to the bottom, glide the page down
  // so the newest line stays in view — no manual scrolling. If they scrolled up, leave
  ...
  useEffect(() => {
    ...
    const id = window.requestAnimationFrame(() => {
      try { sentinel.scrollIntoView({ behavior: 'smooth', block: 'end' }); } catch { sentinel.scrollIntoView(false); }
    });
    return () => window.cancelAnimationFrame(id);
  }, [latest, liveSequence, liveNarration]);
  ...
  // Direct-trigger the TARA call popup the moment the check-in is offered — the user
  // clicks Start inside the popup, never a "start call" button on the task bar. Guarded
  // so dismissing the popup does not immediately re-pop; re-armed once the offer clears.
  useEffect(() => {
    const offering = firstLifeExperience?.phase === 'AWAITING_ADMIN_CHECKIN';
    if (offering && !adminAutoOpenedRef.current) {
      adminAutoOpenedRef.current = true;
      setAdminCheckinOpen(true);
    } else if (!offering) {
      adminAutoOpenedRef.current = false;
    }
  }, [firstLifeExperience?.phase]);
  ...
  return <section className="relative -mx-4 -my-4 min-h-full bg-[#fbfaf7]" aria-label="Company HQ runtime">
    ...
    {playbookApproval?.campaign && playbookApproval.run_id !== dismissedWorkflowApprovalId ? <div className="fixed inset-0 z-[72] overflow-y-auto bg-white/28 px-4 py-8 backdrop-blur-xl sm:px-8" role="dialog" aria-modal="true" aria-label="Choose campaign launch policy">
      <div className="mx-auto flex min-h-full w-full max-w-[1380px] flex-col justify-center">
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-black/15 bg-white/70 px-5 py-4 backdrop-blur-2xl sm:px-7">
          <div><div className="flex items-center gap-2 font-mono text-[9px] uppercase tracking-[0.16em] text-[#525252]"><ShieldCheck size={13} />Campaign ready for launch</div><h3 className="mt-2 text-[22px] font-semibold text-[#171717]">Choose how Runtime should govern these posts</h3><p className="mt-1 max-w-3xl text-[11px] leading-5 text-[#625f58]">The exact captions, visuals, channels, and schedule shown below are persisted. Any change invalidates this authority decision.</p></div>
          <button type="button" onClick={() => setDismissedWorkflowApprovalId(playbookApproval.run_id)} aria-label="Close campaign review" title="Close" className="grid h-9 w-9 place-items-center bg-white/75 text-[#525252] hover:bg-white"><X size={17} /></button>
        </div>
        <div className="bg-white/54 px-1 py-5 backdrop-blur-md sm:px-3"><RuntimeCampaignCanvas campaign={playbookApproval.campaign} /></div>
        <div className="flex flex-wrap items-center justify-end gap-2 border-t border-black/15 bg-white/78 px-5 py-4 backdrop-blur-2xl sm:px-7">{!playbookApproval.manual_only && playbookApproval.preference === 'unconfigured' ? <><button type="button" onClick={() => decidePlaybookAuthority({ preference: 'manual', approve: false })} disabled={Boolean(approvalBusy)} className="h-10 bg-white px-5 text-[11px] font-semibold text-[#525252] shadow-sm disabled:opacity-40">{approvalBusy === 'manual' ? 'Saving...' : 'Review every launch'}</button><button type="button" onClick={() => decidePlaybookAuthority({ preference: 'auto', approve: true })} disabled={Boolean(approvalBusy)} className="inline-flex h-10 items-center gap-2 bg-[#171717] px-5 text-[11px] font-semibold text-white disabled:opacity-40">{approvalBusy === 'auto-send' ? <ArcSpin size={13} /> : <Play size={13} />}Approve and use Auto</button></> : <button type="button" onClick={() => decidePlaybookAuthority({ preference: 'manual', approve: true })} disabled={Boolean(approvalBusy)} className="inline-flex h-10 items-center gap-2 bg-[#171717] px-5 text-[11px] font-semibold text-white disabled:opacity-40">{approvalBusy === 'manual-send' ? <ArcSpin size={13} /> : <Play size={13} />}Approve these posts</button>}</div>
      </div>
    </div> : null}
    {playbookApproval && !playbookApproval.campaign && playbookApproval.run_id !== dismissedWorkflowApprovalId ? <div className="fixed inset-0 z-[72] grid place-items-center bg-black/40 p-3 sm:p-4" role="dialog" aria-modal="true" aria-label={`Choose ${approvalNoun} policy`}>
