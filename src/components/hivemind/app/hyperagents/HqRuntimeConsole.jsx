...
export function AgentRuntimeTasksPanel({ queue, firstLife, experience, onDecision, onAdminDecision, onClose }) {
  ...
  return <section id="agent-runtime-tasks" className="w-full border border-[#171717] bg-[#fbfaf7] shadow-[0_14px_36px_-30px_rgba(0,0,0,0.7)]" aria-label="Agent Runtime tasks">
    ...
    <div className="max-h-[min(46vh,430px)] overflow-y-auto p-2">
      {experience?.phase === 'AWAITING_ADMIN_CHECKIN' ? <article className="mb-2 border border-[#d8d3cc] bg-white px-3 py-3"><div className="font-mono text-[8px] uppercase tracking-[0.12em] text-[#777168]">Before the first plan</div><div className="mt-1 text-[11px] font-semibold text-[#262626]">Add your current company context</div><p className="mt-1 text-[10px] leading-4 text-[#777168]">Talk to Runtime in this browser, or continue using the company evidence already observed.</p><div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => onAdminDecision?.('talk')} className="h-8 bg-[#171717] text-[10px] font-semibold text-white">Talk to Runtime</button><button type="button" onClick={() => onAdminDecision?.('skipped')} className="h-8 border border-[#d8d3cc] text-[10px] font-semibold text-[#525252]">Skip for now</button></div></article> : null}
      {queue.map((item) => {
...
export default function HqRuntimeConsole({ objective, baselineReady }) {
  ...
  const decideAdminCheckin = async (decision, sessionId = null) => {
    ...
    try { await apiClient.decideHqFirstLifeAdminCheckin(decision, sessionId); await load(); }
    catch (requestError) { setError(requestError?.response?.data?.error || requestError?.message || 'The internal check-in could not be recorded.'); }
  ...
  return <section className="relative -mx-4 -my-4 min-h-full bg-[#fbfaf7]" aria-label="Company HQ runtime">
    ...
    {adminCheckinOpen ? <div className="fixed inset-0 z-[75] grid place-items-center bg-black/45 p-4" role="dialog" aria-modal="true" aria-label="Talk to Runtime"><div className="relative w-full max-w-xl"><button type="button" onClick={() => setAdminCheckinOpen(false)} aria-label="Close internal check-in" className="absolute right-3 top-3 z-10 grid h-8 w-8 place-items-center rounded-full bg-white/90 text-[#171717]"><X size={15} /></button><AaasVoiceWidget userId={runtime?.ownerUserId} orgId={runtime?.orgId} provider="grok" initialMode="internal" initialGoal="Listen to the administrator's current company status, priorities, corrections, and blockers. Do not create operating work from the conversation." onSessionCreated={({ sessionId }) => decideAdminCheckin('started', sessionId)} onSessionEnded={({ sessionId }) => { setAdminCheckinOpen(false); decideAdminCheckin('completed', sessionId); }} /></div></div> : null}
