...
export default function HqRuntimeConsole({ objective, baselineReady }) {
  ...
  return <section className="relative -mx-4 -my-4 min-h-full bg-[#fbfaf7]" aria-label="Company HQ runtime">
    ...
    {capabilityRequest && capabilityRequest.id !== dismissedCapabilityRequestId ? <div className="fixed inset-0 z-[70] grid place-items-center bg-black/35 p-3 sm:p-4" role="dialog" aria-modal="true" aria-label={`Connect ${providerLabel(capabilityRequest.provider)}`}><div className={`flex max-h-[calc(100dvh-24px)] w-full flex-col overflow-hidden rounded-[8px] border border-[#d8d3cc] bg-[#fbfaf7] shadow-2xl ${capabilityRequest.prepared_batch ? 'max-w-4xl' : 'max-w-md'}`}><div className="relative shrink-0 border-b border-[#e3e0db] px-5 py-4"><button type="button" onClick={() => setDismissedCapabilityRequestId(capabilityRequest.id)} aria-label="Close connection request" title="Close" className="absolute right-3 top-3 grid h-8 w-8 place-items-center rounded-md text-[#777168] transition-colors hover:bg-[#f0eee9] hover:text-[#171717]"><X size={16} /></button><div className="flex items-center gap-2 pr-9 font-mono text-[9px] uppercase tracking-[0.12em] text-[#525252]"><Cable size={13} />{capabilityRequest.prepared_batch ? 'Outreach prepared' : 'Capability required'}</div><h3 className="mt-3 pr-9 text-[20px] font-semibold text-[#171717]">{capabilityRequest.prepared_batch ? `Your outreach batch is ready. Connect ${providerLabel(capabilityRequest.provider)} to continue.` : `Connect ${providerLabel(capabilityRequest.provider)}`}</h3><p className="mt-2 text-[13px] leading-6 text-[#625f58]">{publicRuntimeText(capabilityRequest.reason)}</p></div>{capabilityRequest.prepared_batch ? <div className="min-h-0 flex-1 overflow-y-auto px-4 py-5 sm:px-7"><AuthorityReviewContent approval={capabilityRequest.prepared_batch} /></div> : null}<div className="shrink-0 border-t border-[#e3e0db] bg-white px-5 py-4"><p className="text-[11px] leading-5 text-[#777168]">The leads, personalization notes, and prepared messages remain attached to this lifecycle. Connecting the provider resumes the same run; nothing is rediscovered.</p><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={async () => { await apiClient.recheckHqCapabilities(); await load(); }} className="h-9 rounded-md border border-[#d8d3cc] px-3 text-[11px] font-semibold text-[#525252]">Check connection</button><button type="button" onClick={openCapability} className="h-9 rounded-md bg-[#171717] px-4 text-[11px] font-semibold text-white">Connect {providerLabel(capabilityRequest.provider)}</button></div></div></div></div> : null}
