...
const QUEUE_DOT = {
  COMPLETED: 'bg-[#c2bcb2]', RUNNING: 'bg-[#171717]', READY: 'bg-[#171717]',
  BLOCKED: 'bg-[#171717]', WAITING_FOR_CONNECTOR: 'bg-[#a8a29a]', WAITING_FOR_EVIDENCE: 'bg-[#a8a29a]',
};
...
function RuntimeTranscript({ events, state, queue = [], liveSequence = null }) {
  ...
  return <div className="relative w-full px-5 py-8 sm:px-8 sm:py-10">
    ...
    <div className="mx-auto grid max-w-[1180px] items-start gap-8 lg:grid-cols-[minmax(0,1fr)_300px]">
      ...
      <aside className="sticky top-[78px] z-10 w-full border border-[#dedbd6] bg-white shadow-[0_10px_30px_-28px_rgba(0,0,0,0.8)]" aria-label="Runtime todo list">
        <div className="border-b border-[#e7e4df] px-4 pb-2.5 pt-3">
          <div className="flex items-center justify-between gap-3"><span className="flex items-center gap-2 font-mono text-[10px] font-semibold uppercase tracking-[0.16em] text-[#171717]"><ListTodo size={13} />Progress</span><span className="font-mono text-[10px] text-[#777168]">{done} of {queue.length}</span></div>
          <div className="mt-2.5 flex h-[3px] w-full bg-[#eeebe5]" role="progressbar" aria-valuenow={progress} aria-valuemin={0} aria-valuemax={100}><span className="h-full bg-[#171717] transition-[width] duration-500" style={{ width: `${progress}%` }} /></div>
        </div>
        <div className="max-h-[min(42vh,360px)] overflow-y-auto">
          {queue.map((item, index) => {
            const [label] = QUEUE_STATUS[item.status] || QUEUE_STATUS.READY;
            const live = item.status === 'RUNNING';
            return <div key={item.id || `${item.title}-${index}`} className={`border-b border-[#eeeae5] px-4 py-3 last:border-b-0 ${live ? 'bg-[#faf9f6]' : ''}`}><div className="flex items-start gap-2.5"><span className={`mt-1 h-2 w-2 shrink-0 rounded-full ${QUEUE_DOT[item.status] || 'bg-[#aaa49c]'}`} style={live ? { animation: 'hm-rt-breathe 1.3s ease-in-out infinite' } : undefined} /><div className="min-w-0 flex-1"><div className={`text-[11px] font-semibold leading-4 ${item.status === 'COMPLETED' ? 'text-[#8a8577]' : 'text-[#262626]'}`}>{item.title}</div><div className="mt-1 font-mono text-[8px] uppercase tracking-[0.12em] text-[#8a8577]">{label}</div>{item.objective ? <p className="mt-1 line-clamp-2 text-[10px] leading-4 text-[#777168]">{item.objective}</p> : null}</div></div></div>;
          })}
          {!queue.length ? <div className="px-4 py-5 text-[11px] text-[#777168]">No queued work.</div> : null}
        </div>
      </aside>
    </div>
  </div>;
...
export default function HqRuntimeConsole({ objective, baselineReady }) {
  ...
  const workflowApproval = (work.workflow_approvals || [])[0] || null;
  const decideWorkflowApproval = async (decision) => {
    if (!workflowApproval || approvalBusy) return;
    setApprovalBusy(decision); setError('');
    try {
      await apiClient.decideHqWorkflowApproval(workflowApproval.workflow_id, decision);
      await load();
    } catch (requestError) {
  ...
  return <section className="relative -mx-4 -my-4 min-h-full bg-[#fbfaf7]" aria-label="Company HQ runtime">
    ...
    <header className="sticky top-0 z-20 border-y border-[#e3e0db] bg-white/95 px-5 py-3 shadow-[0_8px_24px_-22px_rgba(0,0,0,0.45)] backdrop-blur sm:px-8">
      <div className="relative mx-auto flex max-w-5xl items-center justify-between gap-3">
        ...
        <div className="relative flex min-w-0 shrink-0 items-center gap-1.5">
          ...
          <RuntimeButton icon={ListTodo} label="Queue" badge={queueActiveCount || undefined} onClick={() => setQueueOpen((open) => !open)} title="Operating queue and todos" />
    ...
    {workflowApproval && workflowApproval.workflow_id !== dismissedWorkflowApprovalId ? <div className="fixed inset-0 z-[72] grid place-items-center bg-black/35 p-4" role="dialog" aria-modal="true" aria-label="Approve outreach emails"><div className="flex max-h-[min(760px,88vh)] w-full max-w-2xl flex-col overflow-hidden rounded-[8px] border border-[#d8d3cc] bg-[#fbfaf7] shadow-2xl"><div className="relative border-b border-[#e3e0db] px-5 py-4"><button type="button" onClick={() => setDismissedWorkflowApprovalId(workflowApproval.workflow_id)} aria-label="Close outreach approval" title="Close" className="absolute right-3 top-3 grid h-8 w-8 place-items-center rounded-md text-[#777168] transition-colors hover:bg-[#f0eee9] hover:text-[#171717]"><X size={16} /></button><div className="flex items-center gap-2 pr-9 font-mono text-[9px] uppercase tracking-[0.12em] text-[#525252]"><ShieldCheck size={13} />External action approval</div><h3 className="mt-3 pr-9 text-[20px] font-semibold text-[#171717]">{workflowApproval.title || 'Outreach emails are ready'}</h3><p className="mt-2 pr-9 text-[12px] leading-5 text-[#777168]">Review the exact recipients and copy. Approving sends through the organization&apos;s connected Gmail account and starts reply monitoring.</p></div><div className="min-h-0 flex-1 overflow-y-auto p-4"><div className="space-y-3">{(workflowApproval.drafts || []).map((draft, index) => <article key={`${draft.prospect_id || draft.to || 'draft'}-${index}`} className="border border-[#dedbd6] bg-white"><div className="grid gap-2 border-b border-[#ebe8e3] px-4 py-3 sm:grid-cols-[100px_1fr]"><span className="font-mono text-[8px] uppercase tracking-[0.12em] text-[#8a8577]">To</span><span className="truncate text-[11px] font-medium text-[#262626]">{draft.to || draft.recipient}</span><span className="font-mono text-[8px] uppercase tracking-[0.12em] text-[#8a8577]">Subject</span><span className="text-[11px] font-medium text-[#262626]">{draft.subject}</span></div><p className="whitespace-pre-wrap px-4 py-3 text-[12px] leading-5 text-[#525252]">{draft.body || draft.text}</p></article>)}</div></div><div className="flex items-center justify-end gap-2 border-t border-[#e3e0db] bg-white px-5 py-4"><button type="button" onClick={() => decideWorkflowApproval('reject')} disabled={Boolean(approvalBusy)} className="h-9 rounded-md border border-[#d8d3cc] px-4 text-[11px] font-semibold text-[#525252] disabled:opacity-40">{approvalBusy === 'reject' ? 'Rejecting...' : 'Reject'}</button><button type="button" onClick={() => decideWorkflowApproval('approve')} disabled={Boolean(approvalBusy)} className="inline-flex h-9 items-center gap-2 rounded-md bg-[#171717] px-4 text-[11px] font-semibold text-white disabled:opacity-40">{approvalBusy === 'approve' ? <ArcSpin size={13} /> : <Send size={13} />}Approve and send</button></div></div></div> : null}
  </section>;
}
