    ...
    {playbookApproval && playbookApproval.run_id !== dismissedWorkflowApprovalId ? <div className="fixed inset-0 z-[72] grid place-items-center bg-black/35 p-4" role="dialog" aria-modal="true" aria-label={`Choose ${approvalNoun} policy`}>
      <div className="flex max-h-[min(760px,88vh)] w-full max-w-2xl flex-col overflow-hidden rounded-[8px] border border-[#d8d3cc] bg-[#fbfaf7] shadow-2xl">
        <div className="relative border-b border-[#e3e0db] px-5 py-4">
          <button type="button" onClick={() => setDismissedWorkflowApprovalId(playbookApproval.run_id)} aria-label="Close authority request" title="Close" className="absolute right-3 top-3 grid h-8 w-8 place-items-center rounded-md text-[#777168] transition-colors hover:bg-[#f0eee9] hover:text-[#171717]"><X size={16} /></button>
          <div className="flex items-center gap-2 pr-9 font-mono text-[9px] uppercase tracking-[0.12em] text-[#525252]"><ShieldCheck size={13} />External action authority</div>
          <h3 className="mt-3 pr-9 text-[20px] font-semibold text-[#171717]">{playbookApproval.preference === 'unconfigured' ? `The ${approvalNoun} is ready. How should Runtime proceed?` : `The ${approvalNoun} is ready for review`}</h3>
          <p className="mt-2 pr-9 text-[12px] leading-5 text-[#777168]">{playbookApproval.manual_only ? `This action always requires manual approval. Runtime will authorize only the exact immutable call shown here.` : playbookApproval.preference === 'unconfigured' ? `Choose once for this organization. Auto continues future verified ${approvalNoun} gates; Manual pauses every exact batch.` : `Manual review is active. The ${approvalNoun} will not execute until you approve this exact checkpoint.`}</p>
        </div>
