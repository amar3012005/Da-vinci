...
export default function CampaignDetail({ campaign, loading, onBack, onControl, onApproveAction, busy }) {
    ...
    <div className="sticky top-0 z-10 bg-[#fbfaf6]/95 backdrop-blur border-b border-[#dfdbd4]">
      <div className="px-4 sm:px-7 pt-4"><button onClick={onBack} className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-[#6f6962]"><ArrowLeft size={13} />All campaigns</button>
        <div className="mt-3 flex flex-col sm:flex-row sm:items-end gap-3 justify-between"><div><div className="flex items-center gap-2"><h1 className="text-[20px] font-semibold text-[#171717]">{campaign.name}</h1><span className="text-[9px] font-mono uppercase bg-[#efede8] rounded px-1.5 py-0.5">{campaign.status}</span></div><p className="text-[11.5px] text-[#746e67] mt-1 max-w-3xl">{campaign.goal}</p></div>
          <div className="flex gap-2">{campaign.status === 'READY_FOR_APPROVAL' ? <button onClick={() => onControl('approve')} disabled={busy} className="h-9 px-4 bg-[#171717] text-white rounded-md text-[11.5px] font-semibold inline-flex items-center gap-1.5"><ShieldCheck size={13} />Approve plan</button> : null}{['RUNNING', 'SCHEDULED'].includes(campaign.status) ? <button onClick={() => onControl('pause')} disabled={busy} className="h-9 px-3 border border-[#bdb7af] rounded-md text-[11.5px] font-semibold inline-flex items-center gap-1.5"><Pause size={13} />Pause</button> : null}{campaign.status === 'PAUSED' ? <button onClick={() => onControl('resume')} disabled={busy} className="h-9 px-3 border border-[#bdb7af] rounded-md text-[11.5px] font-semibold inline-flex items-center gap-1.5"><Play size={13} />Resume</button> : null}</div>
        </div>
      </div>
