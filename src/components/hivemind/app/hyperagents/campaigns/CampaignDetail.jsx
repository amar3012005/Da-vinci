...
export default function CampaignDetail({ campaign, loading, onBack, onOpenRoom, onControl, onApproveAction, onRetryAction, onReconcileAction, onEditAction, onRemoveAction, onGenerateImage, onUploadImage, onSelectImage, onRemoveImage, onRegenerate, busy, executionEnabled = false, executionBlockers = [] }) {
  ...
  return <div className="h-full overflow-y-auto bg-[#fbfaf6]">
      ...
      <div className="px-4 py-4 sm:px-7">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
          ...
          <div className="flex shrink-0 items-center gap-2 self-end sm:self-auto">
            ...
            {['READY_FOR_APPROVAL', 'NEEDS_INPUT', 'NEEDS_REPAIR', 'FAILED'].includes(campaign.status) ? <button onClick={() => setRegenerateOpen(true)} disabled={busy} className="inline-flex h-9 items-center gap-1.5 rounded-md border border-[#bdb7af] px-3 text-[11px] font-semibold"><RotateCcw size={13} />Regenerate</button> : null}
    ...
    <main className="mx-auto max-w-7xl px-4 py-5 sm:px-7">
      ...
      {campaign.status === 'NEEDS_REPAIR' ? <div className="mb-4 rounded-md border border-amber-300 bg-amber-50 p-4 text-[11.5px] text-amber-900"><strong>The campaign is preserved.</strong> Only affected actions need repair. Accepted posts and their schedule remain unchanged.</div> : null}
