...
export default function ChannelTab({ channel, actions, onApprove, busy }) {
  ...
  return <div>{rows.map((action) => <ActionCard key={action.id} action={action} onApprove={onApprove} busy={busy} />)}</div>;
}
