...
export default function CampaignsView({ onOpenRoom }) {
  ...
  const removeAction = async (actionId) => { if (!window.confirm('Remove this action and create a new campaign plan version?')) return; setBusy(true); setError(''); try { await apiClient.removeCampaignAction(selected.id, actionId); await loadDetail(selected.id); await load(); } catch (err) { setError(err?.response?.data?.message || err.message); } finally { setBusy(false); } };
  const generateImage = async (actionId, payload) => { setBusy(true); setError(''); try { await apiClient.generateCampaignImage(selected.id, actionId, payload); await loadDetail(selected.id); } catch (err) { setError(err?.response?.data?.message || err.message); throw err; } finally { setBusy(false); } };
  const uploadImage = async (actionId, file, altText) => { setBusy(true); setError(''); try { await apiClient.uploadCampaignImage(selected.id, actionId, file, altText); await loadDetail(selected.id); } catch (err) { setError(err?.response?.data?.message || err.message); throw err; } finally { setBusy(false); } };
  const selectImage = async (actionId, assetId) => { setBusy(true); setError(''); try { await apiClient.selectCampaignImage(selected.id, actionId, assetId); await loadDetail(selected.id); } catch (err) { setError(err?.response?.data?.message || err.message); } finally { setBusy(false); } };
  const removeImage = async (actionId, assetId) => { setBusy(true); setError(''); try { await apiClient.removeCampaignImage(selected.id, actionId, assetId); await loadDetail(selected.id); } catch (err) { setError(err?.response?.data?.message || err.message); } finally { setBusy(false); } };
  const regenerate = async (feedback) => { setBusy(true); setError(''); try { await apiClient.regenerateCampaign(selected.id, feedback); await loadDetail(selected.id); await load(); } catch (err) { setError(err?.response?.data?.message || err.message); throw err; } finally { setBusy(false); } };
  ...
  return <div className="h-full overflow-y-auto bg-[#fbfaf6]">
    ...
    {selected || (selectedCampaignId && detailLoading) ? <CampaignDashboardModal campaign={selected} loading={detailLoading} onClose={closeCampaign}>
      {selected ? <CampaignDetail campaign={selected} loading={detailLoading} onBack={closeCampaign} onOpenRoom={onOpenRoom || (() => {})} onControl={control} onApproveAction={approveAction} onRetryAction={(id) => controlAction(id, 'retry')} onReconcileAction={(id) => controlAction(id, 'reconcile')} onEditAction={editAction} onRemoveAction={removeAction} onGenerateImage={generateImage} onUploadImage={uploadImage} onSelectImage={selectImage} onRemoveImage={removeImage} onRegenerate={regenerate} busy={busy} executionEnabled={Boolean(capabilities?.execution_enabled)} /> : null}
