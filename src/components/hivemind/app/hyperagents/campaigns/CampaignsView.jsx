...
const statusTone = { RUNNING: 'text-emerald-700 bg-emerald-50', COMPLETED: 'text-emerald-700 bg-emerald-50', PREPARING_ASSETS: 'text-blue-700 bg-blue-50', FAILED: 'text-red-700 bg-red-50', NEEDS_INPUT: 'text-amber-800 bg-amber-50', NEEDS_REPAIR: 'text-amber-800 bg-amber-50' };
...
export default function CampaignsView({ onOpenRoom }) {
  ...
  return <div className="h-full overflow-y-auto bg-[#fbfaf6]">
    ...
    <main className="grid gap-6 px-4 py-6 sm:px-7 xl:grid-cols-[minmax(0,1fr)_290px]">
      ...
      <section className="mb-7 flex min-h-40 flex-col items-center justify-center rounded-lg border border-[#ddd8d0] bg-white px-5 py-7 text-center shadow-[0_12px_40px_rgba(30,25,20,0.04)]"><span className="grid h-10 w-10 place-items-center rounded-full bg-[#171717] text-white"><Plus size={16} /></span><h2 className="mt-3 text-[16px] font-semibold">Create a campaign</h2><p className="mt-1 max-w-md text-[10.5px] leading-5 text-[#817b74]">Brief your HyperAgents one decision at a time. The Campaign Room researches, challenges, produces, renders, and returns the finished campaign for approval.</p><button onClick={() => setShowCreate(true)} disabled={!capabilities?.enabled} className="mt-4 inline-flex h-10 items-center gap-2 rounded-md bg-[#171717] px-5 text-[11.5px] font-semibold text-white disabled:bg-[#bcb6ae]"><Plus size={13} />Start campaign</button></section>
