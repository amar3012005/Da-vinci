...
const emptyLogs = { mixed: [], runtime: [], hyperagents: [], core: [], control: [], employees: [] };
...
function CommercialManager() {
  ...
  const [enterpriseInvitationForm, setEnterpriseInvitationForm] = useState({ company_name: '', workspace_name: '', recipient_email: '', account_type: 'enterprise_managed', storage_mode: 'hybrid', onboarding_days: 14, invitation_expires_at: '', welcome_message: '', private_notes: '', send_email: true });
  ...
  const [personalInvitationForm, setPersonalInvitationForm] = useState({ invitation_code: '', validity_days: 14 });
  const [personalInvitationUrl, setPersonalInvitationUrl] = useState('');
  const [invitationDetail, setInvitationDetail] = useState(null);
  const [emailForm, setEmailForm] = useState({ template_id: 'welcome_signup', to: '', name: '' });
  ...
  const submitEnterpriseInvitation = async (event) => {
    event.preventDefault(); setError(''); setNotice('');
    try {
      const result = await apiClient.createPlatformEnterpriseInvitation({ ...enterpriseInvitationForm, hosting_mode: enterpriseInvitationForm.account_type === 'enterprise_self_hosted' ? 'self_host' : 'managed', invitation_expires_at: enterpriseInvitationForm.invitation_expires_at ? new Date(enterpriseInvitationForm.invitation_expires_at).toISOString() : undefined });
      ...
      setNotice(result.email_dispatch?.ok === false ? 'Invitation created, but email delivery failed. Use Send to retry.' : 'Enterprise invitation created. Copy the recovery code now; it is not retained.');
      setEnterpriseInvitationForm({ ...enterpriseInvitationForm, company_name: '', workspace_name: '', recipient_email: '', welcome_message: '', private_notes: '' });
      await load();
    } catch (err) { setError(err.response?.data?.error || err.message); }
  };
  const createPersonalInvitationLink = async (event) => {
    ...
    try {
      ...
      setPersonalInvitationForm({ ...personalInvitationForm, invitation_code: '' });
      setNotice('Personal beta invitation link created. The shared code is not included in the URL.');
    } catch (err) { setError(err.response?.data?.error || err.message); }
  };
  const copyInvitationUrl = async (url) => {
    if (!url) return;
    try { await navigator.clipboard.writeText(url); setNotice('Secure invitation link copied.'); }
    catch { setError('Could not copy the invitation link.'); }
  };
  const copyInvitationCode = async (id, code = oneTimeInvitationCode) => {
    if (!code) return;
    try { await navigator.clipboard.writeText(code); if (id) await apiClient.enterpriseInvitationAction(id, 'code-copied'); setNotice('One-time recovery code copied.'); } catch { setError('Could not copy the recovery code.'); }
  };
  const openInvitation = async (id) => {
    try { setInvitationDetail(await apiClient.getPlatformEnterpriseInvitation(id)); } catch (err) { setError(err.response?.data?.error || err.message); }
  };
  const invitationAction = async (id, action) => {
    try {
      const payload = action === 'extend' ? { invitation_expires_at: window.prompt('New invitation expiry (ISO 8601)', '') || undefined } : {};
      if (action === 'extend' && !payload.invitation_expires_at) return;
      const result = await apiClient.enterpriseInvitationAction(id, action, payload);
      if (result.code) { setOneTimeInvitationCode(result.code); setOneTimeInvitationId(id); }
      if (result.activation_url) setOneTimeInvitationUrl(result.activation_url);
      setNotice(action === 'revoke' ? 'Invitation revoked.' : action === 'extend' ? 'Invitation expiry updated.' : action === 'resend' ? 'A fresh secure link was sent.' : 'Invitation updated.');
      await load();
    } catch (err) { setError(err.response?.data?.error || err.message); }
  };
  const previewEmail = async () => {
    setError(''); setNotice(''); setEmailPreviewing(true);
    try { setEmailPreview(await apiClient.previewPlatformEmail(emailForm)); }
    catch (err) { setError(err.response?.data?.error || err.message); }
    finally { setEmailPreviewing(false); }
  };
  const sendEmail = async (event) => {
    event.preventDefault(); setError(''); setNotice(''); setEmailSending(true);
    try {
      const result = await apiClient.sendPlatformEmail(emailForm);
      setNotice(`Email accepted by ${result.provider || 'the configured provider'}${result.delivery_status ? ` (${result.delivery_status})` : ''}.`);
    } catch (err) { setError(err.response?.data?.error || err.message); }
    finally { setEmailSending(false); }
  };
  const tabs = [['plans', 'Plans'], ['promotions', 'Promotions'], ['invitations', 'Invitations'], ['email', 'Email'], ['referrals', 'Referral codes'], ['pilots', 'Pilot organizations'], ['redemptions', 'Redemptions']];
  return <section className="mb-6 border-y border-[#dfddd5] py-5">
    ...
    {tab === 'invitations' && <section className="mt-5 max-w-4xl border-b border-[#dfddd5] pb-6"><p className="font-mono text-[10px] font-bold uppercase tracking-[0.16em] text-[#117dff]">Personal beta invitation</p><p className="mt-2 text-sm text-[#737373]">Create a safe Singulance link for the shared personal beta code. The code is never included in the generated URL.</p><form onSubmit={createPersonalInvitationLink} className="mt-3 flex flex-wrap gap-3"><input required type="password" autoComplete="off" placeholder="Current personal invitation code" value={personalInvitationForm.invitation_code} onChange={(event) => setPersonalInvitationForm({ ...personalInvitationForm, invitation_code: event.target.value })} className="min-w-64 border border-[#d8d6cf] px-3 py-2 text-sm"/><input type="number" min="1" max="90" aria-label="Personal invitation validity in days" value={personalInvitationForm.validity_days} onChange={(event) => setPersonalInvitationForm({ ...personalInvitationForm, validity_days: event.target.value })} className="w-32 border border-[#d8d6cf] px-3 py-2 text-sm"/><button className="bg-[#0a0a0a] px-3 py-2 text-sm font-medium text-white">Create personal link</button></form>{personalInvitationUrl && <div className="mt-3 flex flex-wrap items-center justify-between gap-3 border border-[#bcd5ff] bg-[#f3f8ff] px-3 py-2 text-sm"><span className="min-w-0 truncate font-mono text-xs">{personalInvitationUrl}</span><button type="button" onClick={() => copyInvitationUrl(personalInvitationUrl)} className="shrink-0 border border-[#9fc1f8] px-2 py-1">Copy link</button></div>}</section>}
    {tab === 'invitations' && oneTimeInvitationUrl && <div className="mt-4 flex max-w-4xl flex-wrap items-center justify-between gap-3 border border-[#bcd5ff] bg-[#f3f8ff] px-3 py-2 text-sm"><span className="min-w-0 truncate font-mono text-xs">{oneTimeInvitationUrl}</span><button type="button" onClick={() => copyInvitationUrl(oneTimeInvitationUrl)} className="shrink-0 border border-[#9fc1f8] px-2 py-1">Copy enterprise link</button></div>}
    ...
    {tab === 'invitations' && <><div className="mt-5 max-w-4xl"><p className="mb-3 text-sm text-[#737373]">One invitation creates one enterprise owner workspace. It starts a 14-day unlimited onboarding period; the owner then chooses a paid Runway configuration.</p><form onSubmit={submitEnterpriseInvitation} className="grid gap-3 md:grid-cols-2"><input required placeholder="Company / workspace name" value={enterpriseInvitationForm.company_name} onChange={(e) => setEnterpriseInvitationForm({ ...enterpriseInvitationForm, company_name: e.target.value })} className="border border-[#d8d6cf] px-3 py-2"/><input required type="email" placeholder="Owner email" value={enterpriseInvitationForm.recipient_email} onChange={(e) => setEnterpriseInvitationForm({ ...enterpriseInvitationForm, recipient_email: e.target.value })} className="border border-[#d8d6cf] px-3 py-2"/><input placeholder="Optional HIVEMIND name" value={enterpriseInvitationForm.workspace_name} onChange={(e) => setEnterpriseInvitationForm({ ...enterpriseInvitationForm, workspace_name: e.target.value })} className="border border-[#d8d6cf] px-3 py-2"/><select value={enterpriseInvitationForm.account_type} onChange={(e) => setEnterpriseInvitationForm({ ...enterpriseInvitationForm, account_type: e.target.value, storage_mode: STORAGE_MODE_OPTIONS[e.target.value][0][0] })} className="border border-[#d8d6cf] px-3 py-2"><option value="enterprise_managed">Managed</option><option value="enterprise_self_hosted">Self-hosted</option></select><select value={enterpriseInvitationForm.storage_mode} onChange={(e) => setEnterpriseInvitationForm({ ...enterpriseInvitationForm, storage_mode: e.target.value })} className="border border-[#d8d6cf] px-3 py-2">{STORAGE_MODE_OPTIONS[enterpriseInvitationForm.account_type].map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><input type="datetime-local" aria-label="Invitation expires" value={enterpriseInvitationForm.invitation_expires_at} onChange={(e) => setEnterpriseInvitationForm({ ...enterpriseInvitationForm, invitation_expires_at: e.target.value })} className="border border-[#d8d6cf] px-3 py-2"/><input type="number" min="1" max="90" value={enterpriseInvitationForm.onboarding_days} onChange={(e) => setEnterpriseInvitationForm({ ...enterpriseInvitationForm, onboarding_days: e.target.value })} className="border border-[#d8d6cf] px-3 py-2" aria-label="Onboarding days"/><label className="flex items-center gap-2 text-sm text-[#525252]"><input type="checkbox" checked={enterpriseInvitationForm.send_email} onChange={(e) => setEnterpriseInvitationForm({ ...enterpriseInvitationForm, send_email: e.target.checked })}/> Send from support@singulancelabs.com</label><textarea placeholder="Company welcome message (optional)" value={enterpriseInvitationForm.welcome_message} onChange={(e) => setEnterpriseInvitationForm({ ...enterpriseInvitationForm, welcome_message: e.target.value })} className="min-h-20 border border-[#d8d6cf] px-3 py-2"/><textarea placeholder="Private operator notes" value={enterpriseInvitationForm.private_notes} onChange={(e) => setEnterpriseInvitationForm({ ...enterpriseInvitationForm, private_notes: e.target.value })} className="min-h-20 border border-[#d8d6cf] px-3 py-2"/><button className="bg-[#117dff] px-3 py-2 font-medium text-white">Create invitation</button></form>{oneTimeInvitationCode && <div className="mt-3 flex items-center justify-between gap-3 border border-amber-200 bg-amber-50 px-3 py-2 text-sm"><span>Recovery code: <strong className="font-mono">{oneTimeInvitationCode}</strong></span><button type="button" onClick={() => copyInvitationCode(oneTimeInvitationId, oneTimeInvitationCode)} className="border border-amber-300 px-2 py-1">Copy once</button></div>}</div><div className="mt-6 divide-y divide-[#e5e2dc]">{enterpriseInvitations.map((invitation) => <div key={invitation.id} className="flex flex-wrap items-center justify-between gap-3 py-3 text-sm"><div><strong>{invitation.company_name}</strong><span className="ml-2 text-[#737373]">{invitation.recipient_email} · {invitation.account_type === 'enterprise_self_hosted' ? 'Self-hosted' : 'Managed'} · {invitation.status}</span><p className="mt-1 text-xs text-[#737373]">Code {invitation.code_hint} · sent {when(invitation.last_sent_at)} · invitation expires {when(invitation.invitation_expires_at)} · {invitation.entitlement ? `${invitation.entitlement.status} until ${when(invitation.entitlement.ends_at)}` : 'Not redeemed'}</p></div><div className="flex flex-wrap gap-2"><button onClick={() => openInvitation(invitation.id)} className="border border-[#d8d6cf] px-2 py-1">History</button>{['draft', 'sent'].includes(invitation.status) && <><button onClick={() => invitationAction(invitation.id, invitation.status === 'draft' ? 'send' : 'resend')} className="border border-[#d8d6cf] px-2 py-1">{invitation.status === 'draft' ? 'Send' : 'Resend'}</button><button onClick={() => invitationAction(invitation.id, 'extend')} className="border border-[#d8d6cf] px-2 py-1">Extend</button><button onClick={() => invitationAction(invitation.id, 'revoke')} className="border border-red-200 text-red-700 px-2 py-1">Revoke</button></>}{invitation.entitlement?.grant_id && <button onClick={() => { setTab('pilots'); setNotice('Use Pilot organizations to create the next immutable entitlement version.'); }} className="border border-[#d8d6cf] px-2 py-1">Amend future entitlement</button>}</div></div>)}</div>{invitationDetail && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"><section className="max-h-[80vh] w-full max-w-2xl overflow-auto bg-white p-5 shadow-xl"><div className="flex items-center justify-between"><h3 className="font-semibold">{invitationDetail.invitation.company_name} invitation history</h3><button onClick={() => setInvitationDetail(null)} className="border px-2 py-1">Close</button></div><p className="mt-2 text-sm text-[#525252]">{invitationDetail.invitation.recipient_email} · {invitationDetail.invitation.status}</p><div className="mt-4 divide-y">{(invitationDetail.audit || []).map((entry) => <div key={entry.id} className="py-2 text-xs text-[#525252]"><strong>{entry.eventType}</strong><span className="ml-2">{when(entry.createdAt)}</span></div>)}</div></section></div>}</>}
    {tab === 'email' && <section className="mt-5 overflow-hidden border border-[#d8d6cf] bg-white font-['Space_Grotesk']"><div className="grid lg:grid-cols-[0.9fr_1.1fr]"><form onSubmit={sendEmail} className="p-6 sm:p-8"><p className="font-mono text-[11px] font-bold uppercase tracking-[0.22em] text-[#117dff]">Transactional email</p><h3 className="mt-3 text-2xl font-semibold tracking-normal text-[#0a0a0a]">Send a polished welcome</h3><p className="mt-2 max-w-md text-sm leading-6 text-[#737373]">From <strong className="font-medium text-[#0a0a0a]">welcome@admin.singulancelabs.com</strong>. Use Invitations for secure enterprise activation links and recovery codes.</p><div className="mt-7 space-y-4"><label className="block"><span className="font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-[#737373]">Template</span><select value={emailForm.template_id} onChange={(event) => { setEmailForm({ ...emailForm, template_id: event.target.value }); setEmailPreview(null); }} className="mt-2 w-full rounded-[6px] border border-[#d8d6cf] bg-white px-3 py-3 text-sm text-[#0a0a0a] outline-none focus:border-[#117dff] focus:ring-1 focus:ring-[#117dff]"><option value="welcome_signup">Welcome to HIVEMIND</option><option value="welcome_login">Welcome back to HIVEMIND</option></select></label><label className="block"><span className="font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-[#737373]">Recipient email</span><input required type="email" value={emailForm.to} onChange={(event) => { setEmailForm({ ...emailForm, to: event.target.value }); setEmailPreview(null); }} placeholder="owner@company.com" className="mt-2 w-full rounded-[6px] border border-[#d8d6cf] px-3 py-3 text-sm text-[#0a0a0a] outline-none placeholder:text-[#a3a3a3] focus:border-[#117dff] focus:ring-1 focus:ring-[#117dff]"/></label><label className="block"><span className="font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-[#737373]">Recipient name</span><input value={emailForm.name} onChange={(event) => { setEmailForm({ ...emailForm, name: event.target.value }); setEmailPreview(null); }} placeholder="Optional" className="mt-2 w-full rounded-[6px] border border-[#d8d6cf] px-3 py-3 text-sm text-[#0a0a0a] outline-none placeholder:text-[#a3a3a3] focus:border-[#117dff] focus:ring-1 focus:ring-[#117dff]"/></label></div><div className="mt-7 flex flex-wrap gap-3"><button type="button" onClick={previewEmail} disabled={emailPreviewing} className="border border-[#0a0a0a] px-4 py-3 font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-[#0a0a0a] disabled:opacity-50">{emailPreviewing ? 'Rendering...' : 'Preview email'}</button><button disabled={emailSending} className="bg-[#117dff] px-4 py-3 font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-white disabled:opacity-50">{emailSending ? 'Sending...' : 'Send email'}</button></div></form><div className="min-h-[480px] bg-[#0d0d0f] p-5 sm:p-7"><div className="flex items-start justify-between gap-3"><div><p className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-[#24d2ed]">Message preview</p><p className="mt-2 text-sm text-[#b5b5b9]">{emailPreview ? emailPreview.subject : 'Render a server-owned template before sending.'}</p></div><span className="border border-[#2f3035] px-2 py-1 font-mono text-[10px] uppercase tracking-[0.14em] text-[#a9aaae]">Email v1</span></div><div className="mt-6 h-[380px] overflow-hidden border border-[#303138] bg-[#f8fafc] shadow-[0_14px_45px_rgba(0,0,0,0.25)]">{emailPreview ? <iframe title="Email preview" sandbox="" srcDoc={emailPreview.html} className="h-full w-full border-0 bg-white"/> : <div className="flex h-full flex-col justify-between p-6"><div><div className="h-1 w-12 bg-[#24d2ed]"/><p className="mt-8 font-mono text-[11px] uppercase tracking-[0.18em] text-[#117dff]">HIVEMIND / System message</p><h4 className="mt-4 text-2xl font-semibold text-[#0a0a0a]">Your welcome message will appear here.</h4></div><p className="font-mono text-[10px] uppercase tracking-[0.14em] text-[#737373]">Preview uses the exact server template</p></div>}</div></div></div></section>}
