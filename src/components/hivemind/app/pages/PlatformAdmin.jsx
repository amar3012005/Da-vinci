...
function CommercialManager() {
  ...
  const load = async () => {
    try {
      const [nextPlans, nextPromotions, nextPilots, nextRedemptions, nextOrganizations] = await Promise.all([apiClient.listPlatformPlans(), apiClient.listPlatformPromotions(), apiClient.listPlatformPilots(), apiClient.listPlatformRedemptions(), apiClient.listPlatformOrganizations()]);
      setPlans(nextPlans.plans || []); setPromotions(nextPromotions.promotions || []); setPilots(nextPilots.pilots || []); setRedemptions(nextRedemptions.redemptions || []); setOrganizations(nextOrganizations.organizations || []);
