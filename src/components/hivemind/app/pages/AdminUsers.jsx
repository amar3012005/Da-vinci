...
function InviteModal({ orgId, teams, projects, onClose, onInvited }) {
  ...
  async function handleSend() {
    ...
    setError(null);
    try {
      const result = await apiClient.createInvite(orgId, {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-[15px] font-semibold text-[#0a0a0a]">Invite Member</h2>
          <button onClick={onClose} className="text-[#737373] hover:text-[#0a0a0a]">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-[12px] font-medium text-[#525252] mb-1">Email</label>
            <input
              type="email"
              value={email}
              onChange={e => setEmail(e.target.value)}
              placeholder="name@company.com"
              className="w-full px-3 py-2 text-[13px] border border-[#e3e0db] rounded-lg focus:outline-none focus:border-[#117dff]"
            />
          </div>

          <div>
            <label className="block text-[12px] font-medium text-[#525252] mb-2">Roles</label>
                ...
                {teams.map(team => (
                  <label key={team.id} className="flex items-center gap-3 cursor-pointer">
                    <input
                      ...
                      className="w-4 h-4 accent-[#117dff]"
                    />
                    <span className="text-[13px] text-[#0a0a0a]">{team.name}</span>
                  </label>
                ))}
              </div>
            </div>
